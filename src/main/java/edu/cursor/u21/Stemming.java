...
public class Stemming {
    ...
    public static Stemming stemmingFactory() {
        return new Stemming();
    }
    ...
    boolean hasSuffix(String word, String suffix, EmptyString stem) {
        ...
        for (int i = 0; i < word.length() - suffix.length(); i++)
            stem.setStr(stem.getStr() + word.charAt(i));
        tmp = stem.getStr();
    ...
    String step1(String str) {
        ...
        EmptyString stem = EmptyString.createEmptyString();
        ...
        if (hasSuffix(str, "eed", stem)) {
            ...
            if ((hasSuffix(str, "ed", stem)) || (hasSuffix(str, "ing", stem))) {
                if (containsVowel(stem.getStr())) {

                    String tmp = "";
                    for (int i = 0; i < stem.getStr().length(); i++)
                        tmp += str.charAt(i);
                    str = tmp;
                    if (str.length() == 1)
                        return str;
    ...
    String step2(String str) {
        ...
        for (int index = 0; index < suffixes.length; index++) {
            if (hasSuffix(str, suffixes[index][0], stem)) {
                if (measure(stem.getStr()) > 0) {
                    str = stem.getStr() + suffixes[index][1];
                    return str;
                }
            }
        }
    ...
    public String stripPrefixes(String word) {
        ...
        String[] prefixes = {"sub", "mid", "pre", "epi", "tri", "uni", "com", "con", "non", "dis", "mis", "mega", "para", "fore", "ante", "semi", "omni", "anti", "magn", "pico", "kilo", "homo", "mono", "nano", "post", "infra", "extra", "intra", "ultra", "pseudo", "milli", "therm", "macro", "micro", "hyper", "super", "inter", "contra", "trans", "circum"};
        String[] shortPrefixes = {"a", "co", "de", "em", "en", "ex", "il", "im", "in", "ir", "re", "un"};
        ...
        if (checkNumber == 0) {
            for (int j = 0; j < shortPrefixes.length; j++) {
                if (word.startsWith(shortPrefixes[j])) {
                    word = word.replaceFirst(shortPrefixes[j], "");
                }
            }
        }
        return word;
